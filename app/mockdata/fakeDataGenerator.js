...
function getRandomDuration() {
    ...
    let durationMins = randomNumber <= 10 ? 30 :    // 30 minutes with 10% chance
                       randomNumber <= 35 ? 60 :    // 1 hour with 25% chance
                       randomNumber <= 60 ? 90 :    // 1.5 hours with 25% chance
                       randomNumber <= 75 ? 120 :   // 2 hours with 15% chance
                       randomNumber <= 85 ? 150 :   // 2.5 hours with 10% chance
                       randomNumber <= 90 ? 180 :   // 3 hours with 5% chance
...
function generateLink() {
    const randomNumber = faker.number.int({ min: 1, max: 100 });
    return randomNumber <= 40 ? "Demo generate link" :
           randomNumber <= 60 ? "Customer will provide link" :
...
function getRandomJobStatus() {
    const randomNumber = faker.number.int({ min: 1, max: 100 });
    return randomNumber <= 20 ? "Open" :
           randomNumber <= 40 ? "Offered" :
           randomNumber <= 50 ? "Assign" :
           randomNumber <= 95 ? "Confirm" :
           "Cancelled";
}
...
function generateAPICall(id) {
    ...
    const startDate = faker.date.between("2024-05-06T00:00:00.000Z", "2024-05-10T23:59:59.999Z");
    ...
    apiCall.expectedEndDate = dayjs(formattedStartDate).add(durationMins, "minute").toISOString().replace(".000Z", "Z");

    return apiCall;
}

// Main script execution logic to generate mock API calls
const numAPICalls = 1;                                 // Number of API calls to generate
const startingId = 10001;                              // Starting ID for API calls
